...
const uint8_t EXPECTED_HOP    = 2;
const char   *LOG_FILE        = "node3_log.csv";
...
void logLine(const char *text)
{
  ...
  {
    Serial.println("⚠️ Node3: SD write failed");
  }
}
...
void handleControl(const PacketFields& fields)
{
  ...
  Serial.print("📡 Node3 forwarded CTRL -> ");
...
void handleData(PacketFields& fields, int rssi, float snr)
{
  ...
  {
    Serial.println("⚠️ Node3: unexpected SRC/HOP");
    return;
  }
  ...
  Serial.print("📤 Node3 -> Node4 total: ");
...
void setup()
{
  ...
  for (int attempt = 0; attempt < 5 && !sdReady; ++attempt)
  {
    ...
    if (!sdReady)
    {
      Serial.println("⚠️ Node3: SD init failed, retrying...");
      delay(200);
    }
  }
  ...
  applyTestConfig(0);
  LoRa.receive();
  Serial.println("✅ LoRa listening for Node 2.");
...
void loop()
{
  ...
  if (packetSize > 0)
  {
    ...
    {
      Serial.println("⚠️ Node3: failed to parse packet");
      LoRa.receive();
      return;
    }
  ...
  if (millis() - lastHeartbeat > 60000UL)
  {
    ...
    Serial.print("ℹ️ Node3 heartbeat | RX:");
    Serial.print(rxCount);
    Serial.print(" TX:");
